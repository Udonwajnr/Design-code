...
// LivePreview — no SSR (uses browser APIs)
      ...
      {[
        ...
        <div
          ...
          style={{
            ...
            background:
              "linear-gradient(90deg, #13131f 0%, #1e1e30 50%, #13131f 100%)",
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center" }}>
        ...
        >
          Upload a design → configure → Generate
          <br />
...
export default function OutputPanel({
  ...
  // Viewport state lives here — persists across tab switches
  const [viewport, setViewport] = useState("desktop");
  const [refreshKey, setRefreshKey] = useState(0);
  const [isDark, setIsDark] = useState(true);

  // Track theme for preview
  useEffect(() => {
    const update = () =>
  ...
  return (
    ...
    >
      ...
      {/* ── Streaming / done status bars ── */}
      {status === "streaming" && (
        <StreamingBar tokenCount={tokenCount} onCancel={onCancel} />
      )}
      {status === "done" && <DoneBar onReset={onReset} />}

      {/* ── Viewport toolbar — only shown on preview tab when done ── */}
      ...
      >
        ...
        {/* PREVIEW TAB — LivePreview, only rendered when there's final code */}
