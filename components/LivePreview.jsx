...
// ─── Main component ───────────────────────────────────────────────────────────
export default function LivePreview({ code, viewport, isDark, bare = false }) {
  ...
  useEffect(() => { if (code) inject(code) }, [isDark]) // eslint-disable-line

  // Listen for iframe messages
  useEffect(() => {
    const handler = (e) => {
      if (e.data?.type === 'preview-ready') setState('ready')
      if (e.data?.type === 'preview-error') { setState('error'); setErrorMsg(e.data.message) }
    }
  ...
  useEffect(() => {
    if (!containerRef.current || !currentViewport.frameWidth) { setScale(1); return }
  ...
  return (
    <div ref={containerRef} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', overflow:'auto', background:'var(--bg-secondary)', position:'relative' }}>

      {/* Loading overlay */}
      {state === 'loading' && (
        <div style={{ position:'absolute', inset:0, zIndex:10, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(8,8,16,0.55)', backdropFilter:'blur(4px)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'12px 20px', borderRadius:'10px', background:'var(--bg-card)', border:'1px solid var(--border-default)' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--accent-primary)" strokeWidth="2.5" strokeLinecap="round" style={{ animation:'spin 0.8s linear infinite' }}>
              <path d="M12 2a10 10 0 0 1 10 10"/>
            </svg>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:'12px', color:'var(--text-secondary)' }}>Rendering preview…</span>
          </div>
      ...
      {state === 'error' && errorMsg && (
        <div style={{ width:'100%', padding:'10px 16px', background:'rgba(239,68,68,0.06)', borderBottom:'1px solid rgba(239,68,68,0.2)', display:'flex', alignItems:'flex-start', gap:'8px', flexShrink:0 }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--error)" strokeWidth="2" strokeLinecap="round" style={{ flexShrink:0, marginTop:'1px' }}>
            <circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/>
          </svg>
          <span style={{ fontFamily:'var(--font-mono)', fontSize:'11px', color:'var(--error)', lineHeight:'1.5' }}>{errorMsg}</span>
        </div>
      )}

      {/* Device frame — skipped in bare/compare mode */}
      {bare ? (
        ...
        <div style={{
          width: currentViewport.frameWidth ? `${currentViewport.frameWidth}px` : '100%',
          maxWidth: '100%',
          display: 'flex',
          ...
          {/* Browser chrome */}
          <div style={{ height:'36px', flexShrink:0, background:'var(--bg-card)', borderBottom:'1px solid var(--border-subtle)', display:'flex', alignItems:'center', padding:'0 12px', gap:'8px' }}>
            <div style={{ display:'flex', gap:'5px' }}>
              {['#f87171','#fbbf24','#34d399'].map(c => <div key={c} style={{ width:'10px', height:'10px', borderRadius:'50%', background:c, opacity:0.8 }} />)}
            </div>
            <div style={{ flex:1, height:'20px', borderRadius:'5px', background:'var(--bg-secondary)', border:'1px solid var(--border-subtle)', display:'flex', alignItems:'center', padding:'0 8px', gap:'5px' }}>
              <div style={{ width:'7px', height:'7px', borderRadius:'50%', background:'var(--success)', flexShrink:0 }} />
              <span style={{ fontFamily:'var(--font-mono)', fontSize:'9px', color:'var(--text-muted)' }}>localhost:3000 · preview</span>
            </div>
          </div>
          ...
          <iframe
            ...
            title="Component Preview"
            sandbox="allow-scripts allow-same-origin"
            style={{ flex:1, width:'100%', border:'none', minHeight: currentViewport.id !== 'desktop' ? '560px' : '0', background:'transparent' }}
          />
        </div>
      )}
      </div>
